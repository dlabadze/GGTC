...
class SalaryAttachmentImportWizard(models.TransientModel):
    _name = 'salary.attachment.import.wizard'
    _description = 'Import Salary Attachments from Excel'
    ...
    def action_import(self):
        ...
        try:
            ...
            # 0: Employee_ids (identification_id)
            # 1: description
            ...
            # 6: total_amount
            # 7: status
            # 8: no_end_date (checkbox - if True/true, date_end must be empty)
            ...
            for row_idx in range(1, len(sheet_data)):
                try:
                    ...
                    if row == []:
                        continue
                    # Column 0: Employee_ids (identification_id)
                    employee_id_str = str(row[0]).strip() if len(row) > 0 and row[0] else ''
                    # Remove decimal point and everything after it (e.g., "123.0" -> "123")
                    if '.' in employee_id_str:
                        employee_id_str = employee_id_str.split('.')[0]
                    
                    if not employee_id_str:
                        error_lines.append(f'Row {row_idx + 1}: Missing Employee_ids (identification_id)')
                        continue
                    
                    # Search for employee by identification_id
                    employee = self.env['hr.employee'].search([
                        ('identification_id', '=', employee_id_str)
                    ], limit=1)
                    
                    if not employee:
                        error_lines.append(f'Row {row_idx + 1}: Employee not found with identification_id: {employee_id_str}')
                        continue
                    ...
                    # Column 8: no_end_date (checkbox - if True/true, date_end must be empty)
                    no_end_date = False
                    if len(row) > 8 and row[8]:
                        no_end_date_value = str(row[8]).strip().lower()
                        if no_end_date_value in ['true', '1', 'yes', 'y']:
                            no_end_date = True
                            # If no_end_date is True, set date_end to None (empty)
                            date_end = None
                            total_amount = None
                            no_end_date = True
                            _logger.info(f'Row {row_idx + 1}: no_end_date is True, setting date_end to empty')
