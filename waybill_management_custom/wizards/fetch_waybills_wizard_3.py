...
class FetchWaybillswizard3(models.TransientModel):
    _name = 'fetch.waybills.wizard3'
    _description = 'Fetch Waybills wizard3'
    ...
    def fetch_waybills_3(self):
        try:
            ...
            envelope = data.get('soap:Envelope', {}) if isinstance(data, dict) else {}
            body = envelope.get('soap:Body', {}) if isinstance(envelope, dict) else {}
            response_data = body.get('get_waybillsResponse', {}) if isinstance(body, dict) else {}
            result_data = response_data.get('get_waybillsResult', {}) if isinstance(response_data, dict) else {}
            waybill_list = result_data.get('WAYBILL_LIST', {}) if isinstance(result_data, dict) else {}
            waybills = waybill_list.get('WAYBILL', []) if isinstance(waybill_list, dict) else []
            
            if isinstance(waybills, dict):
                waybills = [waybills]
            elif not isinstance(waybills, list):
                waybills = []
            ...
            for waybill_data in waybills:
                if not isinstance(waybill_data, dict):
                    _logger.warning(f"Skipping invalid waybill data (not a dict): {waybill_data}")
                    continue

                waybill_number_raw = waybill_data.get('WAYBILL_NUMBER', '')
                waybill_number = str(waybill_number_raw or '').strip()
                ...
                waybill_values = {
                    ...
                    'status': waybill_data.get('STATUS', ''),
