...
class Waybill(models.Model):
    ...
    status_name = fields.Char(compute='_compute_status_name', string='სტატუსის დასახელება', store=True)
    ...
    def _compute_status_name(self):
        ...
        for record in self:
            record.status_name = status_map.get(record.status, 'უცნობი სტატუსი')
    xarjang = fields.Many2one('account.account', string='ხარჯის ანგარიში')
    ...
    refund_id = fields.Many2one('account.move', string='Refund')
    ...
    def _get_tax_ids(self, vat_type, tax_type='sale'):
        # Map move_type to tax_type if needed
        if tax_type in ['out_invoice', 'out_refund', 'sale']:
            tax_type = 'sale'
        elif tax_type in ['in_invoice', 'in_refund', 'purchase']:
            tax_type = 'purchase'
            
        # Fetch the tax based on the vat_type
        # RS.GE documentation: 0 is 18%, 1 is 0%, rest is nothing
        if str(vat_type) == '0':  # VAT type 0 corresponds to 18% tax
            tax = self.env['account.tax'].search([
                ('name', '=', '18%'),
                ('type_tax_use', '=', tax_type)
            ], limit=1)
        elif str(vat_type) == '1':  # VAT type 1 corresponds to 0% tax
            tax = self.env['account.tax'].search([
                ('name', '=', '0%'),
                ('type_tax_use', '=', tax_type)
            ], limit=1)
        else:
            tax = self.env['account.tax']  # Default to no taxes
    ...
    def _get_single_tax_id(self, vat_type, tax_type='sale'):
        ...
        if str(vat_type) == '0':  # 18% tax
            tax = self.env['account.tax'].search([
                ('name', '=', '18%'),
                ('type_tax_use', '=', tax_type)
            ], limit=1)
        elif str(vat_type) == '1':  # 0% tax
            tax = self.env['account.tax'].search([
                ('name', '=', '0%'),
                ('type_tax_use', '=', tax_type)
            ], limit=1)
