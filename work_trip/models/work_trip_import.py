...
class SalaryPaymentImport(models.Model):
    ...
    line_ids = fields.One2many('work.trip.import.line', 'import_id', string='Payment Lines')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('work.trip.import') or 'New'
    ...
    def action_import_excel(self):
        ...
        if not HAS_PANDAS:
            raise UserError(
                _("Excel import feature is not available. Please contact your system administrator to install the required components."))
        ...
        for row_idx in range(1, len(df)):
            try:
                ...
                self.env['work.trip.import.line'].create(line_vals)
                total_processed += 1
        ...
        for line in self.line_ids:
            try:
                ...
                if line.credit_partner_id:
                    ...
                    _logger.info("Using credit partner %s with account %s" % (line.credit_partner_id.name,
                                                                              credit_account.code if credit_account else 'None'))
                ...
                _logger.info(
                    f"Project name: {line.project_name}, Analytic account: {line.analytic_account_id.name if line.analytic_account_id else 'None'}")
                ...
                move_vals = {
                    ...
                    'line_ids': [
                        ...
                        (0, 0, {
                            ...
                            'debit': amount,
                            'credit': 0.0,
                            'project_id': line.credit_partner,
                        ...
                        (0, 0, {
                            ...
                            'partner_id': None if is_company_partner else (
                                credit_partner_for_entry.id if credit_partner_for_entry else None),
                ...
                _logger.info(
                    "Created journal entry %s: Debit %s / Credit %s for %s - Project: %s - Company Partner: %s - Analytic: %s" % (
                        counter, debit_account.code, credit_account.code, amount, line.project_name or 'N/A',
                        is_company_partner, analytic_distribution))
...
class SalaryPaymentImportLine(models.Model):
    _name = 'work.trip.import.line'
    _description = 'Salary Payment Import Line'

    import_id = fields.Many2one('work.trip.import', required=True, ondelete='cascade')
