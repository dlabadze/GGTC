...
class AccountMove(models.Model):
    ...
    status = fields.Char('Status')
    combined_invoice_id = fields.Many2one('combined.invoice.model', string='Combined Invoice Model')
    invoice_number = fields.Char(related='combined_invoice_id.invoice_number', string='ზედნადების ნომერი', readonly=False)
    invoice_id = fields.Char(related='combined_invoice_id.invoice_id', string='ზედნადების ID')
    factura_num = fields.Char(related='combined_invoice_id.factura_num', string='ფაქტურის id', readonly=False)
    get_invoice_id = fields.Char(related='combined_invoice_id.get_invoice_id', string='ფაქტურის ნომერი')

    @api.onchange('invoice_number', 'factura_num')
    def _onchange_invoice_fields(self):
        """Create or update combined_invoice_id when invoice_number or factura_num are edited"""
        for record in self:
            if record.invoice_number or record.factura_num:
                if not record.combined_invoice_id:
                    # Create new combined_invoice_id record
                    record.combined_invoice_id = self.env['combined.invoice.model'].create({
                        'invoice_number': record.invoice_number or False,
                        'factura_num': record.factura_num or False,
                        'get_invoice_id': record.factura_num or False,
                    })
                else:
                    # Update existing combined_invoice_id record
                    record.combined_invoice_id.write({
                        'invoice_number': record.invoice_number or False,
                        'factura_num': record.factura_num or False,
                        'get_invoice_id': record.factura_num or False,
                    })
...
class StockMove(models.Model):
    ...
    @api.onchange('unit_price', 'tax_id', 'quantity', 'state', 'tax_included')
    def _compute_cost_including_tax(self):
        for line in self:
            # Check if tax is already included via manual boolean or tax configuration
            if line.tax_id:
                if line.tax_included or line.tax_id.price_include:
    ...
    def create(self, vals):
        ...
        # Removed _set_tax_from_product() - onchange handles it now
    ...
    def _set_price_and_tax_from_order_line(self, order_line, order_type):
        ...
        if taxes:
            ...
            # If order line has no tax, fallback to product tax
            self._set_tax_from_product()
...
class PurchaseOrder(models.Model):
    ...
    invoice_number = fields.Char(related='combined_invoice_id.invoice_number', string='ზედნადების ნომერი', readonly=False)
    invoice_id = fields.Char(related='combined_invoice_id.invoice_id', string='ზედნადების ID')
    factura_num = fields.Char(related='combined_invoice_id.factura_num', string='ფაქტურის ნომერი', readonly=False)
    start_location = fields.Char('ტრანსპორტირების დაწყების ადგილი')
    end_location = fields.Char(string="End Location")
    editable_end_location = fields.Char(string="ტრანსპორტირების დასრულების ადგილი")
    error_field = fields.Char(string="error_field")
    ...
    def _prepare_invoice(self):
        """Override _prepare_invoice to transfer combined_invoice_id with factura_num to invoice"""
        invoice_vals = super(PurchaseOrder, self)._prepare_invoice()
        
        # Ensure combined_invoice_id is created/updated with invoice_number and factura_num
        if self.invoice_number or self.factura_num:
            if not self.combined_invoice_id:
                # Create new combined_invoice_id record
                self.combined_invoice_id = self.env['combined.invoice.model'].create({
                    'invoice_number': self.invoice_number or False,
                    'factura_num': self.factura_num or False,
                    'get_invoice_id': self.factura_num or False,  # Set get_invoice_id to same value as factura_num
                })
            else:
                # Update existing combined_invoice_id record
                self.combined_invoice_id.write({
                    'invoice_number': self.invoice_number or False,
                    'factura_num': self.factura_num or False,
                    'get_invoice_id': self.factura_num or False,  # Set get_invoice_id to same value as factura_num
                })
        
        # Transfer combined_invoice_id to invoice
        if self.combined_invoice_id:
            invoice_vals['combined_invoice_id'] = self.combined_invoice_id.id
