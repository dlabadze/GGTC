...
class StockMove(models.Model):
    ...
    def generate_goods_list_xml(self):
        ...
        for move in self.move_ids_without_package:
            ...
            if quantity == 0:
                raise UserError("რაოდენობა ვერ იქნება ნულის ტოლი product: %s" % product.name)
            # Use cost_including_tax if available, otherwise calculate from unit_price
            amount = move.cost_including_tax if move.cost_including_tax else (move.unit_price * quantity if move.unit_price else 0)
            barcode = product.barcode if product.barcode else ''  # Use empty string if barcode is missing
            ...
            vat_type = -1  # or any other default value
            price_unit = (move.cost_including_tax / quantity) if move.cost_including_tax and quantity else (move.unit_price if move.unit_price else 0)
      ...
      #      raise UserError("ზედნადები უკვე ატვირთულია")
        ...
        for record in self:
            try:
                ...
                # Check if the element was found and assign its text to the company_review field
                if result_element is not None:
                    record.driver_name = result_element.text
                
                # Define the URL and headers for service check
                url = "http://services.rs.ge/WayBillService/WayBillService.asmx"
                headers = {
                    "Content-Type": "text/xml; charset=utf-8",
                }
                ...
                # Find the `un_id` element in the response
                un_id_element = root.find('.//ns:un_id', namespaces)
                seller_un_id = un_id_element.text if un_id_element is not None else ''
                ...
                if Status >= '0':
                    ...
                    # Link the combined_invoice to current model
                    record.combined_invoice_id = combined_invoice.id
                ...
                # Log error but don't try to set company_review (doesn't exist on stock.picking)
                _logger.exception(f"Error during waybill upload: {str(e)}")
                raise
...
class AccountMove(models.Model):
    ...
    def save_invoice_momsaxureba(self, rs_acc, rs_pass, tin):
        ...
        # Get comment from invoice, use empty string if not set
        note = self.comment or ""
    ...
    def save_invoice_desc_momsaxureba(self, rs_acc, rs_pass,factura_num):
        ...
        for index, line in enumerate(self.invoice_line_ids):
            # Use product name if available, otherwise use line name
            ...
            amount = line.price_total
            # tax_id = line.tax_ids.name # This might be unsafe if multiple taxes, but keeping existing logic context
    ...
    def generate_goods_list_xml_1(self, line):
        """Generate XML for a single line item, skipping lines that were likely already on a waybill"""
        # A line should be processed here if:
        # 1. It doesn't have a product (text line)
        # 2. It is a service product (services never exist on waybills)
        
        is_service = line.product_id and line.product_id.type == 'service'
        is_text_line = not line.product_id
        
        if not (is_service or is_text_line):
            _logger.info(f'Skipping product line (likely on waybill): {line.name}')
            return None
        ...
        quantity = line.quantity
        # Only check quantity for non-service lines if needed, 
        # but the user said "no quantities" for services.
        if not is_service and quantity == 0:
        ...
        price_unit = line.price_unit
        amount = line.price_total # quantity * price_unit might differ if there are manual overrides or taxes
    ...
    def faqturebi(self):
        ...
        if invoice_id:
            ...
            # Update the factura with correct operation_date using Revenue service
            # Get required IDs
            un_id, s_user_id = self.rs_un_id(rs_acc, rs_pass)
            buyer_un_id = self.un_id_from_tin(rs_acc, rs_pass, buyer_tin)
            
            # Extract operation date from invoice
            if self.invoice_date:
                doc_date = self.invoice_date
            elif self.date:
                doc_date = self.date
            ...
            # Convert to datetime format (date only, time set to 00:00:00)
            if isinstance(doc_date, str):
                doc_date = fields.Date.from_string(doc_date)
            
            # Format as datetime with timezone (hours don't matter, just date)
            doc_datetime = datetime.combine(doc_date, datetime.min.time())
            tz = timezone(timedelta(hours=4))
            doc_datetime_tz = doc_datetime.replace(tzinfo=tz)
            formatted_datetime = doc_datetime_tz.strftime('%Y-%m-%dT%H:%M:%S%z')
            formatted_datetime = f"{formatted_datetime[:-2]}:{formatted_datetime[-2:]}"

            # Update factura with correct operation_date via Revenue service
            url = "http://www.revenue.mof.ge/ntosservice/ntosservice.asmx"
            headers = {
                "Content-Type": "text/xml; charset=utf-8",
                "SOAPAction": "http://tempuri.org/save_invoice"
            ...
            response = requests.post(url, data=body, headers=headers)
            _logger.info(f'Updated factura {out_inv_id} with operation_date: {formatted_datetime}')
